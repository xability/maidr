...
class Display {
    ...
    toggleBrailleMode(onoff) {
        ...
        if (onoff == "on") {
            if (constants.chartType == "boxplot") { // braille mode is on before any plot is selected
                if (constants.plotOrientation != "vert" && position.x == -1 && position.y == plot.plotData.length) {
                    position.x += 1;
                    position.y -= 1;
                } else if (constants.plotOrientation == "vert" && position.x == 0 && position.y == plot.plotData[0].length - 1) {
    ...
    displayValues(plot) {
        ...
        if (constants.chartType == "barplot") {
            ...
            if (isOutlier || (constants.navigation && constants.plotOrientation == "horz") || (!constants.navigation && constants.plotOrientation == "vert")) {
        ...
        if (constants.chartType == "heatmap") {
            ...
            for (let i = 0; i < plot.plotData[plotPos].length; i++) {
                ...
                if (i == 0) {
                    ...
                    charData = {};
                    let minVal = constants.plotOrientation == "vert" ? constants.minY : constants.minX;
...
class BoxPlot {
    ...
    constructor() {
        ...
        if ( document.querySelector('g[id^="panel"] > g[id^="geom_boxplot.gTree"]') ) {
            constants.plotId = document.querySelector('g[id^="panel"] > g[id^="geom_boxplot.gTree"]').getAttribute('id');
        }
    ...
    CleanData() {
        ...
        if (constants.plotOrientation == "vert") {
            constants.minY = 0;
            constants.maxY = 0;
            for (let i = 0; i < this.plotData.length; i++) { // each plot
                for (let j = 0; j < this.plotData[i].length; j++) { // each section in plot
            ...
            for (let j = 0; j < sections.length; j++) { // each segment (outlier, whisker, etc)
                ...
                for (let k = 0; k < segments.length; k++) {
                    ...
                    for (let l = 0; l < segmentPoints.length; l += 2) {
                        if (segmentType == 'whisker' && l == 0 && constants.plotOrientation == "vert") {
                        } else {
...
class BoxplotRect {
    ...
    UpdateRect() {
        ...
        if ((constants.plotOrientation == "vert" && position.y > -1) || (constants.plotOrientation == "horz" && position.x > -1)) { // initial value could be -1, which throws errors, so ignore that
...
document.addEventListener('DOMContentLoaded', function (e) { // we wrap in DOMContentLoaded to make sure everything has loaded before we run anything
    ...
    if (typeof (constants.chartType) !== "undefined") {
                ...
                constants.autoplayId = setInterval(function () {
                    ...
                    if (dir == "left" || dir == "right" || dir == "up" || dir == "down") {
                        if (
                            ...
                            (constants.plotOrientation == "vert" && dir == "up" && position.y > plot.plotData[position.x].length - 2) ||
                            (constants.plotOrientation == "horz" && dir == "up" && position.y > plot.plotData.length - 2) ||
                            (constants.plotOrientation == "horz" && dir == "right" && position.x > plot.plotData[position.y].length - 2) ||
                            (constants.plotOrientation == "vert" && dir == "right" && position.x > plot.plotData.length - 2) ||
                            (constants.plotOrientation == "horz" && dir == "down" && position.y < 1) ||
                            (constants.plotOrientation == "vert" && dir == "down" && position.y < 1)
                        ) {
                            doneNext = true;
                        }
                    } else {
        ...
        } else if (constants.chartType == "scatterplot" || constants.chartType.includes("scatterplot")) {
