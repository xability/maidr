...
plt.title('Normalized Passenger Count by Class and Survival Status on the Titanic')
plt.xlabel('Class')
plt.ylabel('Number of Passengers')
plt.xticks(rotation=0)

# Save the plot as an SVG file
plt.savefig('stacked_normalized.svg', format='svg')
...
with open("stacked_normalized_raw_data.json", 'w') as json_file:
    json.dump(maidr["data"], json_file, indent=4)

# Writing the dictionary to a JSON file
with open("stacked_normalized_maidr.json", 'w') as json_file:
