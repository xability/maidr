...
export class Context implements Disposable {
  ...
  public getInstruction(includeClickPrompt: boolean): string {
    ...
    switch (state.type) {
      ...
      case 'trace': {
        const groupCountText = state.plotType === 'multiline' && state.groupCount
          ? ` with ${state.groupCount} groups`
          : '';
        return `This is a maidr plot of type: ${state.plotType}${groupCountText}. ${clickPrompt} Use Arrows to navigate data points. Toggle B for Braille, T for Text, S for Sonification, and R for Review mode.`;
      }
    }
  }
}
