...
export class AudioService implements Observer<PlotState>, Disposable {
  ...
  public update(state: PlotState): void {
    ...
    if (audio.isContinuous) {
      ...
      // A trace whose chord spans several x positions (e.g. scatter ROW
      // mode, one note per x) supplies xPerTone parallel to values; every
      // other trace leaves it out and the whole chord shares panning.x.
      const { x: panX, xPerTone } = audio.panning;
      const panAt = (i: number): number => xPerTone?.[i] ?? panX;
