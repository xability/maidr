...
export abstract class AudioManager {
    ...
    private endChime: Node | null = null;
    ...
    protected playSmooth({
      ...
      private createAndStartOscillator(wave: OscillatorType, freqArr: number[], startTime: number, duration: number): OscillatorNode {
        const oscillator = this.audioContext.createOscillator();
        oscillator.type = wave;
      ...
      private createGainNode(gainArr: number[], startTime: number, duration: number): GainNode {
      ...
      private createStereoPanner(panningArr: number[], startTime: number, duration: number): StereoPannerNode {
        const stereoPanner = this.audioContext.createStereoPanner();
        stereoPanner.pan.setValueCurveAtTime(panningArr, startTime, duration);
        return stereoPanner;
      }
      
      private connectAudioNodes(oscillator: OscillatorNode, gain: GainNode, stereoPanner: StereoPannerNode, panner: PannerNode): void {
      ...
      private scheduleCleanup(oscillator: OscillatorNode, panner: PannerNode, duration: number): number {
        return setTimeout(() => {
          panner.disconnect();
    ...
    public abstract playTone(...args: any[]): void;
