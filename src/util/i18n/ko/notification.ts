...
export const notification = {
  ...
  'notification.deltaCandlestickOnly': '기준선 비교는 캔들스틱 차트에서만 사용할 수 있습니다.',
  'notification.deltaNeedsLineLayer': '기준선 비교는 선 레이어가 있는 캔들스틱 차트에서만 사용할 수 있습니다.',
  'notification.deltaReferenceUnavailable': '선택한 기준선을 사용할 수 없습니다.',
  'notification.deltaNoMatchingX': '캔들스틱 차트와 {reference} 사이에 일치하는 x 값이 없습니다.',
  ...
  'notification.deltaActivated': '기준선 비교 켜짐. OHLC 가격에서 {reference|을를} 뺀 값이며 데이터 점은 {count}개, {field}에서 시작합니다. 양수는 선 위, 음수는 선 아래를 뜻합니다. 왼쪽과 오른쪽 화살표로 캔들 사이를 이동하고, 위와 아래 화살표로 시가, 고가, 저가, 종가를 전환하세요. Alt L 키로 비교를 끄고, G 키로 극값을 찾고, 로터로 선 위, 선 아래, 선 위치의 점을 살펴볼 수 있습니다. Escape 키를 누르면 차트로 돌아갑니다.',
  ...
  'notification.deltaYAxisLabel': '{axis} 차이',
