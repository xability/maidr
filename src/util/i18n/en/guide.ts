/**
 * Messages of the chart type guide: the collapsible section of the description
 * dialog that explains, in plain words, what kind of chart is being described.
 *
 * A reader who has never seen a chart cannot be assumed to know what a
 * "violin plot" or a "treemap" is, and the rest of the dialog -- axes, stats,
 * a data table -- means little until they do. Each chart type therefore says
 * three things: what it is, what people use it for, and what it looks like,
 * the last described by shape and position rather than by colour alone.
 *
 * Keys are `guide.<trace type>.<part>`, where the trace type is the
 * `TraceType` value itself, so a chart type added without a guide fails the
 * type check in `chartGuide` rather than rendering an empty section.
 */
export const guide = {
  // Dialog chrome.
