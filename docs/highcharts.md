...
> **Item note:** an `item` chart is a pie drawn as discrete symbols — a parliament diagram is the canonical one, each seat a dot. Highcharts registers it as its own series type and documents it as inheriting from `pie`, and its points carry the same `name` and `y` a pie's do, so it reads through the same converter and announces identically to the pie of the same data. Without that it fell to the dispatch's `default:` and its chart came out with no layers at all (#1138) — the same gap `columnpyramid` and `pictorial` had against `column`, and the same answer: the painting differs, the data does not.
...
> **Choropleth note:** the Highmaps `map` series is the one chart in this list that needs no configuration at all — `map` is a series type of its own, so nothing plainer wears the configuration. What makes it a map rather than a bar chart whose categories happen to be places is the **centroids**: `ChoroplethTrace` walks north, south, east and west out of a longitude and a latitude, and the grammar takes those in **degrees** and in nothing else. The adapter reads them from the two places that state degrees outright — the map feature's own `hc-middle-lon` / `hc-middle-lat`, and the map view's `projectedUnitsToLonLat` applied to the anchor Highcharts placed the shape's label at — and drops any reading that could not be a coordinate. A map with no geographic projection therefore gets no centroids, which is correct: its units are not degrees, and a wrong compass direction is worse than the region list in declared order the grammar sanctions when the pair is missing. `neighbors` is not emitted at all — Highcharts knows which shapes it drew, not which of them share a border, and centroids do not answer it either, so the trace is told nothing about borders rather than something guessed. A region the chart shades as null has no value to announce and is left out; the announced regions are stamped with `data-maidr-region-index` and the selectors address the stamp, so a map with a gap in it still highlights the country it is talking about. The dimensions are named `Region` and `Value`, since a map series is bound to no axis a title could be read from.

> **Price series note:** Highcharts registers three, and MAIDR reads all three as a `candlestick`. `candlestick` and `ohlc` carry an opening price; **`hlc` does not** — measured, an `hlc` point comes back as `{"x":0,"y":3,"low":1,"high":5,"close":3}`, with no `open` and the close on `y`. It was declined outright until #1188, because `CandlestickPoint.open` was required and the converter filtered on it, so dispatching it there would have emitted zero points.
>
> The open is optional now, the way `ErrorBarPoint.y` became optional in #1047 for the band that draws only bounds, and its **absence removes more than a row**. The body is what an open makes, so a chart without one has no bullish/bearish/neutral trend, no candle shape, and no pattern between neighbours — a doji is a body of no height, a hammer a small body over a long wick, an engulfing one body covering another. All of them go together: the trace drops the `open` section, offers no trend rotor unit (a rotor that can never advance is a keyboard trap), leaves `z` off the announcement rather than naming an empty trend, and leaves the Open and Trend columns out of the data table. Nothing is defaulted — an open defaulted to the close would make every candle "Neutral", which is a finding where the truth is that the chart never said.

> **Map marker note:** `mapbubble` and `mappoint` are the `map` series' two siblings, and until #1186 both fell to the adapter's `default:` and emitted no layer — so a chart whose only series was one of them attached MAIDR and gave the reader nothing to navigate. They differ by one field, and that field decides how each reads.
>
> A **map bubble** is a named place with a magnitude and a position, which is `ChoroplethPoint` exactly, so it reads as a **choropleth** — the same call the Chart.js adapter already makes for `chartjs-chart-geo`'s `bubbleMap`, where a row carrying its own longitude and latitude is what turns the region list into the spatial walk. It was declined before on the grounds that announcing its bubbles as shaded regions would report a magnitude the chart draws nowhere, which was wrong twice over: the magnitude *is* drawn, as the bubble's area, and the two adapters were meanwhile giving one chart two different answers. The magnitude is the **size** Highcharts names `z`, taken ahead of any `value` the same point carries: a bubble joined to a map feature can hold both, Highcharts colours it by `value` and sizes it by `z`, and the bubble is what the chart is named for. That ordering also keeps a bubble placed in the map view's projected units honest — such a point carries `y` as a *position*, and the plain `value ?? y` chain announced it as the measurement.
>
> A **map point** is that chart with the magnitude taken away — measured, `{name, lat, lon}` and an `x` that is the marker's index, and no `y` at all. `ChoroplethPoint.y` is required and the only ways to fill it are to invent a constant or to promote the index, both of which announce a measurement the chart never made, so it reads as a **scatter** of degrees east against degrees north with the place name on the point. A marker placed in projected units rather than in degrees states nothing that can be announced as a coordinate; the series is declined with a warning rather than read, so the rest of the chart stays readable.

> **What Highcharts draws and this adapter declines.** Recorded so each is a decision rather than an omission. `vector` and `windbarb` carry a **length and a direction** at every position, and MAIDR has no directional quantity — a reading would drop the direction and announce half the datum with full confidence. `polygon` and `mapline` carry vertices and nothing else; a polygon's canonical use is a hull drawn over a scatter that is already read, so the shape is drawing rather than data. `venn` and `euler` are set-overlap diagrams, and MAIDR has no trace whose navigation is set membership; the decision is recorded once under *Shapes deliberately not read* in `docs/SCHEMA.md` (#1190), and the Chart.js and amCharts sides point at the same place.
...
| --- | --- |
| `venn`, `euler` | A set-overlap diagram, which no trace navigates by set membership. The reason lives under *Shapes deliberately not read* in `docs/SCHEMA.md` (#1190), not here. |
