...
- **Marks whose value is a colour: `cell`, `contour` and `density`.** One exclusion rather than three — Plot puts the magnitude in a fill, and its colour scales are continuous (#1084). A `cell` keeps its magnitude in an 8-bit fill, so several distinct values render as the same colour and no inversion can tell them apart. `Plot.contour` draws one filled band per level and carries the level *only* in the fill; the scale it samples is linear rather than thresholded, so twenty bands come back as twenty samples of a scheme and not as twenty levels. Reading the levels off each band's index and the scale's domain instead would assume Plot spaced its thresholds evenly — an assumption about Plot's internals rather than something the drawing states, and a chart with author-set thresholds would be announced with levels it does not have. `Plot.density` is worse off again: its isolines are drawn `fill="none" stroke="currentColor"`, every one the same colour, so the level is not encoded at all. Announcing an approximation to a reader who cannot check it against the picture is worse than announcing nothing, so all three are skipped.
