...
| `bindD3Dot` | Cleveland dot plot [experimental] | `<circle>` per category | [Dot plot](examples.html) |
| `bindD3Lollipop` | Lollipop [experimental] | `<circle>` head per category | [Lollipop](examples.html) |
| `bindD3Funnel` | Funnel / stage chart [experimental] | `<path>` or `<rect>` per stage | [Funnel](examples.html) |
| `bindD3Line` | Line / multi-line, or a step chart with `stepDirection` | `<path>` per series, optional `<circle>` points | [Line chart](examples.html) |
| `bindD3Area` | Area / stacked area / 100% stacked area [experimental] | `<path>` per series | [Area chart](examples.html) |
| `bindD3Bump` | Bump / rank chart [experimental] | `<path>` per competitor | [Bump chart](examples.html) |
| `bindD3Scatter` | Scatter | `<circle>` per point | [Scatter plot](examples.html) |
| `bindD3Manhattan` | Manhattan plot [experimental] | `<circle>` per marker | [Manhattan](examples.html) |
| `bindD3Volcano` | Volcano plot [experimental] | `<circle>` per gene | [Volcano](examples.html) |
...
| `bindD3ErrorBar` | Error bar / point range [experimental] | `<g>` per estimate (containing `<line>` + marker) | [Error bar](examples.html) |
| `bindD3Forest` | Forest / meta-analysis [experimental] | `<g>` per study, plus `pooledSelector` for the diamond | [Forest](examples.html) |
| `bindD3Boxen` | Boxen / letter-value [experimental] | `<g>` per distribution (the stack of rungs) | [Boxen](examples.html) |
| `bindD3Dumbbell` | Dumbbell / connected dot [experimental] | `<line>` connector per row | [Dumbbell](examples.html) |
| `bindD3Waterfall` | Waterfall / bridge [experimental] | `<rect>` per step | [Waterfall](examples.html) |
...
| `bindD3Diverging` | Diverging bars / population pyramid [experimental] | `<rect>` per bar | [Pyramid](examples.html) |
| `bindD3Mosaic` | Mosaic / marimekko [experimental] | `<rect>` per cell | [Mosaic](examples.html) |
| `bindD3WordCloud` | Word cloud [experimental] | `<text>` per term | [Word cloud](examples.html) |
| `bindD3Gantt` | Gantt / timeline / swimlane [experimental] | `<rect>` per interval | [Gantt](examples.html) |
| `bindD3Gauge` | Gauge / bullet chart [experimental] | the needle or value arc | [Gauge](examples.html) |
...
| `bindD3PolarArea` | Polar area / coxcomb / rose [experimental] | `<path>` per wedge | [Polar area](examples.html) |
| `bindD3Radar` | Radar / spider [experimental] | `<path>` per series | [Radar](examples.html) |
| `bindD3Network` | Force-directed network [experimental] | `<line>` per link | [Network](examples.html) |
| `bindD3Treemap` | Treemap [experimental] | `<rect>` per node | [Treemap](examples.html) |
| `bindD3Sunburst` | Sunburst [experimental] | `<path>` arc per node | [Sunburst](examples.html) |
| `bindD3Icicle` | Icicle [experimental] | `<rect>` band per node | [Icicle](examples.html) |
| `bindD3Sankey` | Sankey [experimental] | `<path>` ribbon per link | [Sankey](examples.html) |
| `bindD3Alluvial` | Alluvial [experimental] | `<path>` ribbon per link | [Alluvial](examples.html) |
| `bindD3Chord` | Chord [experimental] | `<path>` ribbon per chord | [Chord](examples.html) |
| `bindD3Survival` | Kaplan-Meier survival curve [experimental] | `<path>` per arm, plus `censoredSelector` for the ticks | [Survival](examples.html) |
| `bindD3Parallel` | Parallel coordinates [experimental] | `<path>` or `<polyline>` per observation | [Parallel](examples.html) |
| `bindD3Ridgeline` | Ridgeline / joy plot [experimental] | `<path>` density curve per group | [Ridgeline](examples.html) |
| `bindD3Hexbin` | Hexbin density [experimental] | `<path>` hexagon per bin | [Hexbin](examples.html) |
| `bindD3Contour` | Contour / density field [experimental] | `<path>` per level | [Contour](examples.html) |
| `bindD3Choropleth` | Choropleth map [experimental] | `<path>` per region | [Choropleth](examples.html) |
...
### Dot Plot, Lollipop, and Funnel [experimental]
...
A line drawn with one of d3's step curves is a **step chart**, and says so with
`stepDirection`: `'hv'` for `d3.curveStepAfter`, which holds the level and jumps
at the next sample, `'vh'` for `d3.curveStepBefore`, which jumps at the current
one, and `'mid'` for `d3.curveStep`, which jumps halfway between the two. The
chart is then navigated by transition rather than by sample and described in
terms of its runs.
...
Declare it rather than expecting it to be detected: this binder reads your data
off `__data__` and never looks at the path, and the path would not settle it
anyway — a staircase is indistinguishable from a line whose samples happen to
land on one, and which curve you used is your own choice. Leave `stepDirection`
out and the chart is read as the line it was, with no direction claimed; that
is a default, not a limit, and a chart drawn with any of the three curves has a
value to pass. `bindD3Area` takes the same field and stays an area: its trace
reads the convention to tell the risers of a stepped band from its samples.
...
A violin is two layers over one set of marks — the KDE curve a reader walks
along, and the five-number summary the box overlay draws — so one bind produces
both, in one subplot. `selector` matches the mirrored `d3.area()` path per
category, with that category's bins bound to it; `boxSelector` matches the
overlay's groups, when the chart draws one.
...
The bins may be bound to the path directly — what
`.data(groups.map(g => g.bins))` leaves — or wrapped in an object alongside the
category name, in which case `kde` names the field holding them. Either way the
category comes from `fill`, read off the wrapper when there is one and off a bin
when there is not.

The result carries **both** layers: `result.layer` is the KDE, and
`result.layers` is `[kde, box]`. Every other binder returns `layers` too, as a
one-element array, so a caller that wants "what did this bind produce" can read
the same field everywhere.

**The summary is never derived from the curve.** Quartiles can be computed off a
density estimate, and they would be the smoothing bandwidth's rather than the
data's — a reader told "Q1 is 4.2" has no way to tell it was inferred. A violin
whose overlay states no quantiles, or which draws none, is read as its KDE
curves alone. The same goes for a partial summary: if some categories state one
and others do not, the box layer is dropped entirely rather than emitted with
its highlights one category out of step.
...
### Area / Stacked Area / 100% Stacked Area [experimental]
...
### Radar / Spider Chart [experimental]
...
### Polar Area / Coxcomb [experimental]
...
### Diverging Bars / Population Pyramid [experimental]
...
### Error Bars / Point Range [experimental]
...
### Dumbbell / Connected Dot [experimental]
...
### Gantt / Timeline / Swimlane [experimental]
...
### Sankey / Alluvial / Chord [experimental]
...
The step convention is `'hv'` without being asked, because a Kaplan-Meier
estimate holds until an event drops it — `d3.curveStepAfter`. Pass
`stepDirection: 'vh'` for a curve drawn the other way round.
...
### Contour / Density Field [experimental]
...
### Gauge / Bullet Chart [experimental]
