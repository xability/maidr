...
one. The chart is then navigated by transition rather than by sample and
described in terms of its runs.
...
Declare it rather than expecting it to be detected: `d3.curveStep*` leaves
nothing in the rendered path that tells it from a line whose samples happen to
land on a staircase, and which curve you used is your own choice. `d3.curveStep`
itself puts the riser midway between the samples, which is neither convention
and which MAIDR cannot name — leave `stepDirection` out and the chart is read
as the line it was, with no direction claimed. `bindD3Area` takes the same field
and stays an area: its trace reads the convention to tell the risers of a
stepped band from its samples.
