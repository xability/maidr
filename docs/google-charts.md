...
| Network | `OrgChart` (`orgchart` package) — nodes joined by parent pointers | `'OrgChart'` |
...
> **Network note:** an `OrgChart`'s `[node id, parent id, tooltip]` table maps straight onto links — one per row that names a parent, parent first — and the nodes and their degrees are derived from them, so no node list is emitted. Identity comes from the **raw** cell rather than the formatted one, because a parent pointer has to match the id it names and an org chart routinely puts markup in the value it draws. **An OrgChart is never highlighted:** it renders an HTML `<table>` rather than SVG and draws no element per link — the connectors are cell borders — so there is nothing for a selector to point at. Left and right walk a group's nodes most connected first, up and down move between groups, and the rotor follows the links.
