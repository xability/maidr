...
- `type`: the type of plot. The declarable types are `area`, `bar`, `box`, `boxen`, `bump`, `candlestick`, `contour`, `diverging_bar`, `dodged_bar`, `dot`, `dumbbell`, `error_bar`, `forest`, `funnel`, `gantt`, `gauge`, `heat`, `hexbin`, `hist`, `line`, `lollipop`, `manhattan`, `mosaic`, `parallel_coordinates`, `pie`, `point`, `polar_area`, `radar`, `ridgeline`, `smooth`, `stacked_area`, `stacked_bar`, `stacked_normalized_area`, `stacked_normalized_bar`, `step`, `survival`, `treemap`, `violin_box`, `violin_kde`, `volcano`, `waterfall`, `word_cloud`. `TraceType` in `src/type/grammar.ts` is the source of truth; `candlestick_delta` appears there but is built at runtime from a candlestick and a reference line, so it is not something a page declares.
