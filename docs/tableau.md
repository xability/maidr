...
- **A programmatic selection fires the same event as a click.** Every selection and clear the adapter issues is remembered, by position, and an event that resolves to one of them is its echo, consumed and ignored — including an echo that arrives after the cursor has moved on, which would otherwise drag the cursor back through where it had been. A user's click on the mark MAIDR is already on is therefore read as an echo too; nothing is lost, since the cursor is already there.
- **A mark carries no id**, so the position is reconstructed from the mark's field values. A mark whose values two positions share — a point cloud the page declared with `overrides[name].traceType` over rows that repeat a detail value — moves nothing, and so does a multi-mark selection that spans several positions: landing the reader on the wrong one of two is worse than leaving them where they are.
...
- **Stacked versus side-by-side bars is not something the API reports.** Tableau's summary data gives each segment's own value in both layouts; nothing in the numbers says whether they were drawn on top of one another or beside one another, and no property anywhere in the public contract reports Tableau's *Stack Marks* setting. The visual specification does carry the shelves and the encodings, so the layout is not entirely unevidenced — but inferring it from where a field sits would be a claim about Tableau's authoring habits, not a reading of a declared fact, and the adapter does not make it. `dodged_bar` is the default because it announces each group's own value and never claims a total the view may not have drawn. MAIDR's segmented trace appends its synthetic *Total* summary row either way, so the totals are still there for a stacked view — they are simply not asserted as the drawing.
  ...
  The marks card's *Color* encoding does not settle it either, because Tableau's own *Analysis → Stack Marks* menu is a counterexample: take a `bar` card with a dimension on *Color* and on neither shelf, then switch *Stack Marks* from *Automatic* to *Off*. The view goes from stacked bars to overlapping ones, and the visual specification before and after is identical — the same `primitiveType`, the same `encodings`, the same `rowFields` and `columnFields`. Any rule over those members gives both views the same answer, so at least one of them would be announced wrongly. This was re-checked against the latest declarations published at the time of writing, `@tableau/embedding-api@3.16.1` and `@tableau/extensions-api-types@1.17.0`: `VisualSpecification`, `MarksSpecification` and `Encoding` are unchanged from 3.12.1, and nothing anywhere in either contract names stacking. **`overrides[name].traceType` is the only way to say a view is stacked** (`'stacked_bar'`) or normalized (`'stacked_normalized_bar'`), and it applies equally to both binders.
...
| `MarkSelectionChanged` | `markselectionchanged` | a user clicked a mark — MAIDR's cursor follows it (see [Following a click](#following-a-click)); the adapter's own selections arrive as the same event and are ignored |
...
- **A click is followed only when it names one position.** A mark carries no stable id, so the position is reconstructed from the mark's field values; a mark two positions share, or a multi-mark selection spanning several, moves nothing. The API also gives no way to tell a programmatic selection from a user's, so the adapter's own selections are recognised by what they selected — which means a click on the very mark MAIDR is on is read as an echo. See [Following a click](#following-a-click).
