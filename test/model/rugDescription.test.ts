 ...
 *
 * The range stats are labelled after the axis the layer authored ("v range",
 * not "X range"), so the summary and the data table headers below it name the
...
describe('an axis that never moves', () => {
  test('says it is constant rather than printing a span of nothing', () => {
    ...
    expect(statOf(traceOf(RUG), 'Rug range')).toBe('constant 0');
  });
  ...
  test('leaves an axis that does move alone', () => {
    ...
    expect(statOf(traceOf(RUG), 'v range')).toBe('1.5 to 7');
  });
  ...
  test('applies to whichever axis is flat, not to rugs by name', () => {
    ...
    expect(statOf(trace, 'v range')).toBe('constant 3');
    expect(statOf(trace, 'height range')).toBe('1 to 9');
  });
  ...
  test('an ordinary scatter still reads as two spans', () => {
    ...
    expect(statOf(trace, 'v range')).toBe('1 to 4');
