...
/**
 * Page object representing the StackedBarplot plot page
 * Handles all StackedBarplot plot specific interactions and verifications
 */
export class StackedBarplotPage extends BasePage {
   ...
   * @param plotId - ID of the StackedBarplot plot (default: TestConstants.STACKED_BARPLOT_ID)
   */
  constructor(page: Page, plotId: string = TestConstants.STACKED_BARPLOT_ID) {
    super(page);
    this.plotId = plotId;
  }

  /**
   * Navigates to the StackedBarplot plot page
   * @returns Promise resolving when navigation completes
   * @throws StackedBarplotError if navigation fails
   */
  public async navigateToStackedBarplot(): Promise<void> {
    try {
      const absolutePath = '/examples/stacked-barplot.html';
      await this.navigateTo(absolutePath);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new StackedBarplotError(`Failed to navigate to StackedBarplot: ${errorMessage}`);
    }
  }
   ...
   * @throws StackedBarplotError if timeout is reached before the condition is met
   *
   * @example
  ...
  public async verifyPlotLoaded(): Promise<void> {
    try {
      ...
      throw new StackedBarplotError('StackedBarplot plot failed to load correctly');
    }
  }
  ...
  public async activateMaidr(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new StackedBarplotError(`Failed to navigate to bar plot: ${errorMessage}`);
    }
  }
  ...
  public async activateMaidrOnClick(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new StackedBarplotError(`Failed to activate MAIDR by clicking: ${errorMessage}`);
    }
  }
   ...
   * @throws StackedBarplotError if notification text cannot be retrieved
   */
  public async getNotificationText(): Promise<string> {
    ...
    try {
      ...
      throw new StackedBarplotError('Failed to get notification text');
    }
  }
  ...
  public async isTextModeActive(textMode: string): Promise<boolean> {
    ...
    try {
      ...
      if (textMode === TestConstants.TEXT_MODE_TERSE) {
        ...
        throw new StackedBarplotError('Invalid text mode specified');
      }
    } catch (error) {
  ...
  public async isBrailleModeActive(braille_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (braille_mode === TestConstants.BRAILLE_ON) {
        ...
        throw new StackedBarplotError('Invalid braille mode specified');
      }
    } catch (error) {
  ...
  public async isSonificationActive(sonification_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (sonification_mode === TestConstants.SOUND_ON) {
        ...
        throw new StackedBarplotError('Invalid sonification mode specified');
      }
    } catch (error) {
      throw new StackedBarplotError('Failed to check sonification status');
    }
  }
  ...
  public async isReviewModeActive(review_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (review_mode === TestConstants.REVIEW_MODE_ON) {
        ...
        throw new StackedBarplotError('Invalid review mode specified');
      }
    } catch (error) {
   ...
   * @throws StackedBarplotError if title cannot be retrieved
   */
   ...
   * @throws StackedBarplotError if autoplay fails or times out
   *
   * @example
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new StackedBarplotError(`Failed to complete forward autoplay: ${errorMessage}`);
    }
  }
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new StackedBarplotError(`Failed to complete reverse autoplay: ${errorMessage}`);
    }
  }
}
