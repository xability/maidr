...
export class LinePlotPage extends BasePage {
   ...
   * @throws LinePlotError if rotor navigation fails
   */
  public async moveToNextRotorMode(): Promise<void> {
    try {
      ...
      throw new LinePlotError(`Failed to move to next rotor mode: ${errorMessage}`);
    }
  }
  ...
  public async moveToPrevRotorMode(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new LinePlotError(`Failed to move to previous rotor mode: ${errorMessage}`);
    }
  }
   ...
   * @throws LinePlotError if mode cannot be retrieved
   */
  ...
  public async navigateToRotorMode(targetMode: string): Promise<void> {
    ...
    throw new LinePlotError(`Failed to navigate to rotor mode: ${targetMode}`);
  }
   ...
   * @throws LinePlotError if modal fails to open
   */
  public async openGoToExtremaModal(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new LinePlotError(`Failed to open Go To Extrema modal: ${errorMessage}`);
    }
  }
   ...
   * @throws LinePlotError if modal fails to close
   */
  public async closeGoToExtremaModal(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new LinePlotError(`Failed to close Go To Extrema modal: ${errorMessage}`);
    }
  }
   ...
   * @throws LinePlotError if selection fails
   */
  public async selectExtremaTarget(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new LinePlotError(`Failed to select extrema target: ${errorMessage}`);
    }
  }
   ...
   * @throws LinePlotError if unable to get targets
   */
  public async getExtremaTargets(): Promise<string[]> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new LinePlotError(`Failed to get extrema targets: ${errorMessage}`);
    }
  }
   ...
   * @throws LinePlotError if unable to get selected target
   */
  public async getSelectedExtremaTarget(): Promise<string> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new LinePlotError(`Failed to get selected extrema target: ${errorMessage}`);
    }
  }
  ...
  public async goToMaximum(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new LinePlotError(`Failed to navigate to maximum: ${errorMessage}`);
    }
  }
  ...
  public async goToMinimum(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new LinePlotError(`Failed to navigate to minimum: ${errorMessage}`);
    }
  }
}
