...
export class BoxplotVerticalPage extends BasePage {
   ...
   * @param plotId - ID of the Boxplot Vertical (defaults to BOXPLOT_VERTICAL_ID constant)
   */
  constructor(page: Page, plotId: string = TestConstants.BOXPLOT_VERTICAL_ID) {
    super(page);
    this.plotId = plotId;
  }
  ...
  public async navigateToBoxplotVertical(): Promise<void> {
    ...
    // Use absolute path to ensure correct navigation
      const absolutePath = '/examples/boxplot-vertical.html';

      await this.navigateTo(absolutePath);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new BoxplotVerticalError(`Failed to navigate to Boxplot Vertical: ${errorMessage}`);
    }
  }
   ...
   * @throws BoxplotVerticalError if timeout is reached before the condition is met
   *
   * @example
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new BoxplotVerticalError(
  ...
  public async activateMaidrOnClick(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new BoxplotVerticalError(`Failed to activate MAIDR by clicking: ${errorMessage}`);
    }
  }
   ...
   * @throws BoxplotVerticalError if notification text cannot be retrieved
   */
  public async getNotificationText(): Promise<string> {
    ...
    try {
      ...
      throw new BoxplotVerticalError('Failed to get notification text');
    }
  }
  ...
  public async isTextModeActive(textMode: string): Promise<boolean> {
    ...
    try {
      ...
      if (textMode === TestConstants.TEXT_MODE_TERSE) {
        ...
        throw new BoxplotVerticalError('Invalid text mode specified');
      }
    } catch (error) {
  ...
  public async isBrailleModeActive(braille_mode: string): Promise<boolean> {
    ...
    try {
      ...
      } else if (this.plotId === TestConstants.BOXPLOT_HORIZONTAL_ID || this.plotId === TestConstants.BOXPLOT_VERTICAL_ID) {
        return notification_text === TestConstants.BRAILLE_BOXPLOT_STATUS;
      } else {
        throw new BoxplotVerticalError('Invalid braille mode specified');
      }
    } catch (error) {
  ...
  public async isSonificationActive(sonification_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (sonification_mode === TestConstants.SOUND_ON) {
        ...
        throw new BoxplotVerticalError('Invalid sonification mode specified');
      }
    } catch (error) {
      throw new BoxplotVerticalError('Failed to check sonification status');
    }
  }
  ...
  public async isReviewModeActive(review_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (review_mode === TestConstants.REVIEW_MODE_ON) {
        ...
        throw new BoxplotVerticalError('Invalid review mode specified');
      }
    } catch (error) {
   ...
   * @throws BoxplotVerticalError if title cannot be retrieved
   */
   ...
   * @throws BoxplotVerticalError if autoplay fails or times out
   *
   * @example
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new BoxplotVerticalError(`Failed to complete forward autoplay: ${errorMessage}`);
    }
  }
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new BoxplotVerticalError(`Failed to complete reverse autoplay: ${errorMessage}`);
    }
  }
}
