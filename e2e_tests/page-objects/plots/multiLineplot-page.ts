 ...
 * Handles all Multi Lineplot plot specific interactions and verifications
 */
export class MultiLineplotPage extends BasePage {
   ...
   * @param plotId - ID of the Multi Lineplot plot (default: TestConstants.MULTI_LINEPLOT_ID)
   */
  constructor(page: Page, plotId: string = TestConstants.MULTI_LINEPLOT_ID) {
    super(page);
    this.plotId = plotId;
  }
  ...
  public async navigateToLinePlot(): Promise<void> {
    try {
      const absolutePath = '/examples/multi-lineplot.html';
      await this.navigateTo(absolutePath);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new MultiLineplotError(`Failed to navigate to Multi Lineplot: ${errorMessage}`);
    }
  }
   ...
   * @throws MultiLineplotError if timeout is reached before the condition is met
   *
   * @example
  ...
  public async verifyPlotLoaded(): Promise<void> {
    try {
      ...
      throw new MultiLineplotError('Multi Lineplot plot failed to load correctly');
    }
  }
  ...
  public async activateMaidr(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new MultiLineplotError(`Failed to navigate to bar plot: ${errorMessage}`);
    }
  }
  ...
  public async activateMaidrOnClick(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new MultiLineplotError(`Failed to activate MAIDR by clicking: ${errorMessage}`);
    }
  }
   ...
   * @throws MultiLineplotError if notification text cannot be retrieved
   */
  public async getNotificationText(): Promise<string> {
    ...
    try {
      ...
      throw new MultiLineplotError('Failed to get notification text');
    }
  }
  ...
  public async isTextModeActive(textMode: string): Promise<boolean> {
    ...
    try {
      ...
      if (textMode === TestConstants.TEXT_MODE_TERSE) {
        ...
        throw new MultiLineplotError('Invalid text mode specified');
      }
    } catch (error) {
  ...
  public async isBrailleModeActive(braille_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (braille_mode === TestConstants.BRAILLE_ON) {
        ...
        throw new MultiLineplotError('Invalid braille mode specified');
      }
    } catch (error) {
  ...
  public async isSonificationActive(sonification_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (sonification_mode === TestConstants.SOUND_ON) {
        ...
        throw new MultiLineplotError('Invalid sonification mode specified');
      }
    } catch (error) {
      throw new MultiLineplotError('Failed to check sonification status');
    }
  }
  ...
  public async isReviewModeActive(review_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (review_mode === TestConstants.REVIEW_MODE_ON) {
        ...
        throw new MultiLineplotError('Invalid review mode specified');
      }
    } catch (error) {
   ...
   * @throws MultiLineplotError if title cannot be retrieved
   */
   ...
   * @throws MultiLineplotError if autoplay fails or times out
   *
   * @example
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new MultiLineplotError(`Failed to complete forward autoplay: ${errorMessage}`);
    }
  }
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new MultiLineplotError(`Failed to complete reverse autoplay: ${errorMessage}`);
    }
  }
}
