...
export class DodgedBarplotPage extends BasePage {
   ...
   * @param plotId - ID of the Dodged Barplot plot (default: TestConstants.DODGED BARPLOT_ID)
   */
  constructor(page: Page, plotId: string = TestConstants.DODGED_BARPLOT_ID) {
    super(page);
    this.plotId = plotId;
  }
  ...
  public async navigateToDodgedBarplot(): Promise<void> {
    try {
      const absolutePath = '/examples/dodged-barplot.html';
      await this.navigateTo(absolutePath);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new DodgedBarplotError(`Failed to navigate to Dodged Barplot: ${errorMessage}`);
    }
  }
   ...
   * @throws DodgedBarplotError if timeout is reached before the condition is met
   *
   * @example
  ...
  public async verifyPlotLoaded(): Promise<void> {
    try {
      ...
      throw new DodgedBarplotError('Dodged Barplot plot failed to load correctly');
    }
  }
  ...
  public async activateMaidr(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new DodgedBarplotError(`Failed to navigate to bar plot: ${errorMessage}`);
    }
  }
  ...
  public async activateMaidrOnClick(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new DodgedBarplotError(`Failed to activate MAIDR by clicking: ${errorMessage}`);
    }
  }
   ...
   * @throws DodgedBarplotError if notification text cannot be retrieved
   */
  public async getNotificationText(): Promise<string> {
    ...
    try {
      ...
      throw new DodgedBarplotError('Failed to get notification text');
    }
  }
  ...
  public async isTextModeActive(textMode: string): Promise<boolean> {
    ...
    try {
      ...
      if (textMode === TestConstants.TEXT_MODE_TERSE) {
        ...
        throw new DodgedBarplotError('Invalid text mode specified');
      }
    } catch (error) {
  ...
  public async isBrailleModeActive(braille_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (braille_mode === TestConstants.BRAILLE_ON) {
        ...
        throw new DodgedBarplotError('Invalid braille mode specified');
      }
    } catch (error) {
  ...
  public async isSonificationActive(sonification_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (sonification_mode === TestConstants.SOUND_ON) {
        ...
        throw new DodgedBarplotError('Invalid sonification mode specified');
      }
    } catch (error) {
      throw new DodgedBarplotError('Failed to check sonification status');
    }
  }
  ...
  public async isReviewModeActive(review_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (review_mode === TestConstants.REVIEW_MODE_ON) {
        ...
        throw new DodgedBarplotError('Invalid review mode specified');
      }
    } catch (error) {
   ...
   * @throws DodgedBarplotError if title cannot be retrieved
   */
   ...
   * @throws DodgedBarplotError if autoplay fails or times out
   *
   * @example
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new DodgedBarplotError(`Failed to complete forward autoplay: ${errorMessage}`);
    }
  }
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new DodgedBarplotError(`Failed to complete reverse autoplay: ${errorMessage}`);
    }
  }
}
