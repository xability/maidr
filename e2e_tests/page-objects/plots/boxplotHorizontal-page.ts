...
export class BoxplotHorizontalPage extends BasePage {
   ...
   * @param plotId - ID of the Boxplot Horizontal (defaults to BOXPLOT_HORIZONTAL_ID constant)
   */
  constructor(page: Page, plotId: string = TestConstants.BOXPLOT_HORIZONTAL_ID) {
    super(page);
    this.plotId = plotId;
  }
  ...
  public async navigateToBoxplotHorizontal(): Promise<void> {
    ...
    // Use absolute path to ensure correct navigation
      const absolutePath = '/examples/boxplot-horizontal.html';

      await this.navigateTo(absolutePath);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new BoxplotHorizontalError(`Failed to navigate to Boxplot Horizontal: ${errorMessage}`);
    }
  }
   ...
   * @throws BoxplotHorizontalError if timeout is reached before the condition is met
   *
   * @example
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new BoxplotHorizontalError(
  ...
  public async activateMaidrOnClick(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new BoxplotHorizontalError(`Failed to activate MAIDR by clicking: ${errorMessage}`);
    }
  }
   ...
   * @throws BoxplotHorizontalError if notification text cannot be retrieved
   */
  public async getNotificationText(): Promise<string> {
    ...
    try {
      ...
      throw new BoxplotHorizontalError('Failed to get notification text');
    }
  }
  ...
  public async isTextModeActive(textMode: string): Promise<boolean> {
    ...
    try {
      ...
      if (textMode === TestConstants.TEXT_MODE_TERSE) {
        ...
        throw new BoxplotHorizontalError('Invalid text mode specified');
      }
    } catch (error) {
  ...
  public async isBrailleModeActive(braille_mode: string): Promise<boolean> {
    ...
    try {
      ...
      } else if (this.plotId === TestConstants.BOXPLOT_HORIZONTAL_ID || this.plotId === TestConstants.BOXPLOT_HORIZONTAL_ID) {
        return notification_text === TestConstants.BRAILLE_BOXPLOT_STATUS;
      } else {
        throw new BoxplotHorizontalError('Invalid braille mode specified');
      }
    } catch (error) {
  ...
  public async isSonificationActive(sonification_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (sonification_mode === TestConstants.SOUND_ON) {
        ...
        throw new BoxplotHorizontalError('Invalid sonification mode specified');
      }
    } catch (error) {
      throw new BoxplotHorizontalError('Failed to check sonification status');
    }
  }
  ...
  public async isReviewModeActive(review_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (review_mode === TestConstants.REVIEW_MODE_ON) {
        ...
        throw new BoxplotHorizontalError('Invalid review mode specified');
      }
    } catch (error) {
   ...
   * @throws BoxplotHorizontalError if title cannot be retrieved
   */
   ...
   * @throws BoxplotHorizontalError if autoplay fails or times out
   *
   * @example
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new BoxplotHorizontalError(`Failed to complete forward autoplay: ${errorMessage}`);
    }
  }
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new BoxplotHorizontalError(`Failed to complete reverse autoplay: ${errorMessage}`);
    }
  }
}
