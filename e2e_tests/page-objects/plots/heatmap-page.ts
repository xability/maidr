...
export class HeatmapPage extends BasePage {
   ...
   * @param plotId - ID of the Heatmap plot (default: TestConstants.HEATMAP_ID)
   */
  constructor(page: Page, plotId: string = TestConstants.HEATMAP_ID) {
    super(page);
    this.plotId = plotId;
  }
  ...
  public async navigateToHeatmap(): Promise<void> {
    try {
      const absolutePath = '/examples/heatmap.html';
      await this.navigateTo(absolutePath);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new HeatmapError(`Failed to navigate to Heatmap: ${errorMessage}`);
    }
  }
   ...
   * @throws HeatmapError if timeout is reached before the condition is met
   *
   * @example
  ...
  public async activateMaidr(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new HeatmapError(`Failed to navigate to bar plot: ${errorMessage}`);
    }
  }
  ...
  public async activateMaidrOnClick(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new HeatmapError(`Failed to activate MAIDR by clicking: ${errorMessage}`);
    }
  }
   ...
   * @throws HeatmapError if notification text cannot be retrieved
   */
  public async getNotificationText(): Promise<string> {
    ...
    try {
      ...
      throw new HeatmapError('Failed to get notification text');
    }
  }
  ...
  public async isTextModeActive(textMode: string): Promise<boolean> {
    ...
    try {
      ...
      if (textMode === TestConstants.TEXT_MODE_TERSE) {
        ...
        throw new HeatmapError('Invalid text mode specified');
      }
    } catch (error) {
  ...
  public async isBrailleModeActive(braille_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (braille_mode === TestConstants.BRAILLE_ON) {
        ...
        throw new HeatmapError('Invalid braille mode specified');
      }
    } catch (error) {
  ...
  public async isSonificationActive(sonification_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (sonification_mode === TestConstants.SOUND_ON) {
        ...
        throw new HeatmapError('Invalid sonification mode specified');
      }
    } catch (error) {
      throw new HeatmapError('Failed to check sonification status');
    }
  }
  ...
  public async isReviewModeActive(review_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (review_mode === TestConstants.REVIEW_MODE_ON) {
        ...
        throw new HeatmapError('Invalid review mode specified');
      }
    } catch (error) {
   ...
   * @throws HeatmapError if title cannot be retrieved
   */
   ...
   * @throws HeatmapError if autoplay fails or times out
   *
   * @example
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new HeatmapError(`Failed to complete forward autoplay: ${errorMessage}`);
    }
  }
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new HeatmapError(`Failed to complete reverse autoplay: ${errorMessage}`);
    }
  }
}
