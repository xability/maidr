...
export class MultiLayerPlotPage extends BasePage {
   ...
   * @param plotId - ID of the Multi Layer plot (default: TestConstants.MULTI_LAYER_PLOT_ID)
   */
  constructor(page: Page, plotId: string = TestConstants.MULTI_LAYER_PLOT_ID) {
    super(page);
    this.plotId = plotId;
  }

  /**
   * Navigates to the Multi Layer Plot plot page
   * @returns Promise resolving when navigation completes
   * @throws MultiLayerPlotError if navigation fails
   */
  public async navigateToMultiLayerPlot(): Promise<void> {
    try {
      const absolutePath = '/examples/multi-layer.html';
      await this.navigateTo(absolutePath);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new MultiLayerPlotError(`Failed to navigate to Multi Layer Plot: ${errorMessage}`);
    }
  }
   ...
   * @throws MultiLayerPlotError if timeout is reached before the condition is met
   *
   * @example
  ...
  public async verifyPlotLoaded(): Promise<void> {
    try {
      ...
      throw new MultiLayerPlotError('Multi Layer Plot plot failed to load correctly');
    }
  }
  ...
  public async activateMaidr(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new MultiLayerPlotError(`Failed to navigate to bar plot: ${errorMessage}`);
    }
  }
  ...
  public async activateMaidrOnClick(): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new MultiLayerPlotError(`Failed to activate MAIDR by clicking: ${errorMessage}`);
    }
  }
   ...
   * @throws MultiLayerPlotError if notification text cannot be retrieved
   */
  public async getNotificationText(): Promise<string> {
    ...
    try {
      ...
      throw new MultiLayerPlotError('Failed to get notification text');
    }
  }
  ...
  public async isTextModeActive(textMode: string): Promise<boolean> {
    ...
    try {
      ...
      if (textMode === TestConstants.TEXT_MODE_TERSE) {
        ...
        throw new MultiLayerPlotError('Invalid text mode specified');
      }
    } catch (error) {
  ...
  public async isBrailleModeActive(braille_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (braille_mode === TestConstants.BRAILLE_ON) {
        ...
        throw new MultiLayerPlotError('Invalid braille mode specified');
      }
    } catch (error) {
  ...
  public async isSonificationActive(sonification_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (sonification_mode === TestConstants.SOUND_ON) {
        ...
        throw new MultiLayerPlotError('Invalid sonification mode specified');
      }
    } catch (error) {
      throw new MultiLayerPlotError('Failed to check sonification status');
    }
  }
  ...
  public async isReviewModeActive(review_mode: string): Promise<boolean> {
    ...
    try {
      ...
      if (review_mode === TestConstants.REVIEW_MODE_ON) {
        ...
        throw new MultiLayerPlotError('Invalid review mode specified');
      }
    } catch (error) {
   ...
   * @throws MultiLayerPlotError if title cannot be retrieved
   */
   ...
   * @throws MultiLayerPlotError if autoplay fails or times out
   *
   * @example
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new MultiLayerPlotError(`Failed to complete forward autoplay: ${errorMessage}`);
    }
  }
  ...
  options: { timeout?: number; pollInterval?: number } = {},
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw new MultiLayerPlotError(`Failed to complete reverse autoplay: ${errorMessage}`);
    }
  }
  ...
  */
  ...
  /**
   * Gets the inforrmation about current layer
   * Returns the extracted information
   * @throws MultiLayerPlotError if information cannot be retrieved
   */
  public async getCurrentLayerInfo(): Promise<string> {
    const layerInfoSelector
      = `#${TestConstants.MAIDR_INFO_CONTAINER + this.plotId} ${TestConstants.PARAGRAPH}`;
    try {
      return await this.getElementText(layerInfoSelector);
    } catch (error) {
